...
![Google Ngram: vampires vs. zombies](vampireszombies)
...
![Google Ngram: The United States are/The United States is](usis)
...
![Ngram closed shop, open shop, union shop](closedopenunion-google.png)
...
![Chrome inspector screenshot](chromedata)
...
Now export the data to csv (write.csv) and then -- not sure if this would be necessary, actually, and probably could be done in R, but I didn’t know how, so -- transform the data using rawgraphs.io. Just copy-paste the csv data into the screen, hit on stack data, stack on year.
...
And that produced the image in the book. The numbers don't quite match Google ngram --- there the peak of closed shop is 0.0006 and the peak of open shop is 0.000355. On the other hand, the graph on Ngrams on google doesn’t show them right; the open shop peak goes over 0.0004 despite the #.  However, the shape is the same, and the minor differences don't change the argument.
